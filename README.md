...
| AI 推理 | Deepseek-v4（通过 OpenAI 兼容 API）+ FastAPI + LangChain + Neo4j 知识图谱 |
...
| 送礼 | `GET /gifts` `POST /gifts` `GET /gifts/{id}` `POST /gifts/{id}/order` `GET /gifts/{id}/logistics` `POST /gifts/{id}/feedback` `GET /gifts/{id}/feedback` | 记录列表+筛选(按收礼人/场景/状态) / 创建 / 详情 / 下单 / 物流(含时间轴节点) / 提交反馈(送礼方/收礼方) / 反馈列表 |
...
- [x] 礼物包装 AI 推荐 & 定制系统
- [x] 社区故事 & 日历提醒（编辑/删除）& B端企业服务
- [x] 推荐历史压缩存储 & 批量管理
- [x] 日历编辑/删除 & 记忆库筛选检索
- [x] 礼物包装 AI 兜底 & 跨用户权限修复
- [x] 详情页无效 ID 错误处理
- [x] 双向反馈表单（送礼方/收礼方区分）
...
### 开发中
- [ ] 知识图谱 Neo4j 多跳推理（按 GiftGPT知识图谱分类映射表.md）
- [ ] Python giftgpt-ai 服务全接 LangChain 链
