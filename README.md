# GiftGPT — AI 驱动的全链路礼物推荐平台

> 不猜，更懂TA的心意

消除"不知道送什么"的焦虑，让每一份礼物都恰到好处。

---

## 项目简介

GiftGPT 是一个 AI 驱动的礼物推荐平台，基于收礼人性格画像、送礼场景和预算，智能匹配最合适的礼物，并提供从选品、包装、贺卡到物流的全链路闭环服务。

- **核心用户**：18-30 岁学生及职场新人
- **扩展用户**：30-45 岁中青年（母亲节、教师节等代际送礼）
- **B端场景**：企业团建福利、员工生日、客户礼品

---

## 核心功能

- **收礼人画像**：手动选择性格标签（开朗、文艺、极客等 20+ 维度）+ AI 多模态分析
- **AI 智能匹配**：`性格特征 × 场景 × 预算` → 排序礼物清单 + 推荐理由
- **全链路闭环**：购买跳转 → 包装定制 → AI 贺卡 → 物流追踪 → 双向反馈
- **礼物社区**：成功案例分享、日历提醒、礼物记忆库
- **B端服务**：企业批量团购、员工关怀日历

---

## 技术栈

| 层次 | 技术 |
|------|------|
| 前端 | React 18 + Next.js 14 + TypeScript + TailwindCSS（支持深色模式） |
| 后端 | Spring Boot 2.7 + Java 11（运行）/ JDK 17（编译）+ Maven 多模块 |
| ORM | MyBatis-Plus |
| 认证 | Sa-Token (JWT) |
| 数据库 | H2 嵌入式文件数据库（`data/giftgpt.mv.db`，持久化存储） |
| API 文档 | Knife4j (Swagger) |
| AI 推理 | Deepseek-v4（通过 OpenAI 兼容 API） |
| 商品数据源 | 京东联盟 / 淘宝联盟 / 拼多多多多进宝 官方 API（可配置） |

> **注意**：后端用 JDK 17 编译为 Java 11 字节码，运行时使用 JDK 11。<br>
> JDK 17 在部分 Windows 11 上存在 WEPoll bug，导致嵌入式服务器无法启动；JDK 11 的 WindowsSelectorProvider 可正常工作。<br>
> 数据库文件 `backend/giftgpt-server/data/giftgpt.mv.db` 在服务器重启后依然存在，数据不会丢失。<br>
> SaToken 默认使用内存 token-store，**后端进程重启会使已签发的 token 失效**，前端需重新登录；业务数据（用户、画像、故事等）不受影响。<br>
> `schema.sql` 以 `CREATE TABLE IF NOT EXISTS` + `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` 方式执行，自动修补旧库缺失的列（如 `story_reply.update_time`、`recipient.mbti/personality/recent_purchases`），`continue-on-error` 保证幂等。<br>
> `start-server.bat` 启动时若发现 jar 缺失会自动 `mvn package`，并以 `--spring.profiles.active=local` 启动，加载 `application-local.yml` 中的电商 API 密钥（该文件已 gitignore，需自备）。

---

## 第三方平台 API 配置

GiftGPT 通过联盟推广 API 获取京东、淘宝、拼多多的真实商品数据（商品名、价格、图片、购买链接）。**三个平台均支持个人开发者注册，无需企业营业执照**。API 密钥为**可选配置**——未配置时相应平台会被跳过，不影响其他功能。

> **本项目为商业竞赛用途。** 各平台"联盟 API"是面向推广者的 CPS（按成交付费）接口，个人注册即可，不要求企业资质。

### 推荐流程

```
用户发起推荐 → Deepseek 生成礼物建议 → 按关键词在本地 DB 中匹配真实商品
     ↑                                                    ↓
  选品搜索 ←── 缓存到本地 DB ←── 并行调用三方平台 API（京东联盟 / 淘宝联盟 / 多多进宝）
```

### 配置方式

在 `backend/giftgpt-server/src/main/resources/application-local.yml` 中填入密钥（该文件已加入 `.gitignore`，不会提交到仓库）：

```yaml
giftgpt:
  ai:
    deepseek:
      api-key: sk-xxx    # Deepseek API 密钥
  commerce:
    jingdong:
      app-key:            # 京东联盟 App Key
      app-secret:         # 京东联盟 App Secret
    taobao:
      app-key:            # 淘宝联盟 App Key（纯数字）
      app-secret:         # 淘宝联盟 App Secret
      adzone-id:          # 淘宝联盟推广位 ID（纯数字，必填）
    pinduoduo:
      client-id:          # 多多进宝 Client ID
      client-secret:      # 多多进宝 Client Secret
```

启动时需激活 `local` profile：`--spring.profiles.active=local`

### 各平台注册指南（个人开发者，无需企业资质）

---

#### 一、京东联盟 — 个人注册流程

> **个人可以注册，不需要营业执照。** 使用京东账号实名认证即可。

**网址**：https://union.jd.com

**Step 1 — 注册京东联盟**
1. 打开 https://union.jd.com ，用**个人京东账号**登录（就是你平时购物的京东号）
2. 完成**个人实名认证**（填写姓名、手机号、身份证号），无需营业执照
3. 签署《京东联盟开放平台协议》

**Step 2 — 创建应用，获取 App Key / App Secret**
1. 登录后进入"联盟产品" → "开放平台"（https://union.jd.com/openplatform）
2. 点击"应用管理" → "创建应用"
3. 选择 **"JOS平台"**，填写应用名称（如 GiftGPT），上传应用图标
4. 创建完成后，在应用详情页即可看到 **App Key** 和 **App Secret**
5. 进入"推广管理" → 创建推广位（网站/导购媒体），记录推广位 ID

**Step 3 — 申请接口权限**
1. 打开 https://union.jd.com/openplatform/groupApply
2. 选择推广模式（如"导购类"），提交申请，审批通过后自动开通
3. 在"接口权限"中搜索 `jd.union.open.goods.query`，申请开通

**Step 4 — 填入配置**
```yaml
jingdong:
  app-key: <你的 App Key>
  app-secret: <你的 App Secret>
```

> **限额**：个人开发者每日约 2,000 次，足够比赛演示使用。

---

#### 二、淘宝联盟 — 个人注册流程

> **个人可以注册，不需要营业执照。** 淘宝号直接登录，注册最简单、审批最快。

**网址**：https://www.alimama.com（阿里妈妈）+ https://open.taobao.com（淘宝开放平台）

**Step 1 — 注册阿里妈妈（淘宝联盟）**
1. 打开 https://www.alimama.com
2. 用**个人淘宝账号**登录（就是你平时淘宝购物的号）
3. 完成**个人实名认证**：手机号 + 支付宝实名绑定
4. 选择"我是淘宝客"，确认协议，完成入驻

**Step 2 — 注册淘宝开放平台应用**
1. 打开 https://open.taobao.com ，用**同一个淘宝号**登录
2. 进入"控制台" → "我的应用" → "创建应用"
3. 选择"网页应用"，填写应用名称（如 GiftGPT）和回调地址（http://localhost:3000）
4. 创建后在"应用概览"中获取 **App Key**（纯数字，如 `28325281`）和 **App Secret**

**Step 3 — 获取推广位 adzone_id（必填！）**
1. 回到阿里妈妈后台 alimama.com → "推广管理" → "媒体管理" → "推广位管理"
2. 新建推广位，类型选"导购"
3. 获取推广位 ID，即 **adzone_id**（纯数字，如 `123456789`）

**Step 4 — 申请 API 权限**
1. 在淘宝开放平台应用详情 → "API 权限"标签
2. 搜索 `taobao.tbk.dg.material.optional` → 点击"申请"
3. 通常**秒批**通过，无需等待

**Step 5 — 填入配置**
```yaml
taobao:
  app-key: <你的 App Key（纯数字）>
  app-secret: <你的 App Secret>
  adzone-id: <你的推广位 ID（纯数字）>
```

> **限额**：个人每日约 10,000 次，QPS 限制 10/s。淘宝联盟只返回已加入淘宝客推广（有佣金）的商品。

#### 淘宝常见问题
- **Q: App Key 必须是纯数字？** A: 是的，淘宝联盟的 App Key 是纯数字字符串，如 `28325281`
- **Q: 图片链接打不开？** A: 淘宝返回的图片 URL 可能被浏览器拦截；前端已用 `referrerPolicy="no-referrer"` 处理
- **Q: 搜索不到商品？** A: 联盟只返回已加入推广的商品，部分品牌旗舰店商品不参与淘宝客推广
- **Q: adzone_id 是必填吗？** A: 是的，淘宝联盟要求必填 `adzone_id`，否则接口会报错

---

#### 三、拼多多多多进宝 — 个人注册流程

> **个人可以注册，不需要营业执照。** 选择"个人开发者"身份，应用审核秒过。

**网址**：https://open.pinduoduo.com（拼多多开放平台）+ https://jinbao.pinduoduo.com（多多进宝）

**Step 1 — 注册拼多多开放平台**
1. 打开 https://open.pinduoduo.com → 点击"注册"
2. 使用手机号注册，填写验证码、密码
3. 选择 **"我是第三方开发者"** → 身份类型选择 **"个人开发者"**（不需要营业执照！）
4. 填写个人身份信息（姓名、手机号、身份证号）
5. 点击"提交审核"，等待审核通过（通常几小时）

**Step 2 — 注册多多进宝**
1. 打开 https://jinbao.pinduoduo.com
2. 使用**同一手机号**登录，手机验证码即可自动注册
3. 完成实名认证

**Step 3 — 创建应用，获取 client_id / client_secret**
1. 回到拼多多开放平台 → "我的应用" → "创建应用"
2. 应用类型选择 **"多多客联盟"**
3. 填写应用名称（如 GiftGPT）和说明
4. PRD/MRD 文档：下载模板，导出为 PDF 上传即可（**目前自动审核，基本秒过**）
5. 回调地址填写：`http://www.pinduoduo.com` 或 `http://localhost:3000`
6. 提交后一般**秒通过**审核
7. 在应用详情中获取 **client_id** 和 **client_secret**

**Step 4 — 绑定并创建推广位（关键！）**
1. 进入多多进宝后台（jinbao.pinduoduo.com）→ "API权限"菜单
2. 将上一步获取的 **client_id** 填入，点击**绑定**（这一步必须做，否则 API 不可用！）
3. 在多多进宝后台创建推广位（至少一个）

**Step 5 — 申请接口权限**
1. 在拼多多开放平台应用详情 → "接口权限"
2. 搜索 `pdd.ddk.goods.search` → 申请
3. 审核通过（通常 1-2 个工作日）

**Step 6 — 填入配置**
```yaml
pinduoduo:
  client-id: <你的 Client ID>
  client-secret: <你的 Client Secret>
```

> **限额**：个人开发者每日 2,000 次。拼多多的价格单位是**分**（如 `29900` = ¥299），代码已自动转换。

#### 拼多多常见问题
- **Q: client_id 必须绑定？** A: 是的，必须在多多进宝后台的"API权限"中绑定 client_id，否则调用 API 会失败
- **Q: PRD/MRD 文档不会写？** A: 下载模板后直接导出为空白 PDF 上传即可，目前自动审核秒过，不会有人看内容
- **Q: 返回的价格不对？** A: `min_group_price` 的单位是分（如 `29900` = ¥299.00），`PddService` 已自动转换
- **Q: 签名错误？** A: 拼多多的签名规则是参数值直接拼接无分隔符（`key1value1key2value2`），和其他平台不同，`PlatformApiSigner` 已封装

---

#### 四、得物

得物不提供公开搜索 API（需企业资质 + OAuth 2.0），且无联盟推广接口。得物商品通过种子数据补充。

---

### 三平台注册对比总结

| 对比项 | 京东联盟 | 淘宝联盟 | 多多进宝 |
|--------|----------|----------|----------|
| 注册地址 | https://union.jd.com | https://www.alimama.com | https://open.pinduoduo.com |
| 企业资质 | **不需要** | **不需要** | **不需要** |
| 注册方式 | 京东号 + 实名认证 | 淘宝号 + 支付宝 | 手机号 + 身份证 |
| 审核速度 | 约 1 工作日 | 即时 | 几小时到 2 天 |
| 日调用限额 | 2,000 次 | 10,000 次 | 2,000 次 |
| 推荐指数 | ★★★★ | ★★★★★ | ★★★★ |

> **竞赛建议**：优先注册**淘宝联盟**——注册最快（秒批）、调用额度最高（10,000次/天）、商品数据最丰富。京东联盟和多多进宝审核时间稍长，建议同步申请。

### 测试 API 是否配置成功

启动后端后，用 curl 测试：

```bash
# 搜索所有平台（已配置的平台会返回结果，未配置的自动跳过）
curl "http://localhost:8080/api/v1/products/search?keyword=耳机"

# 仅搜索京东
curl "http://localhost:8080/api/v1/products/search?keyword=机械键盘&platform=京东"

# 仅搜索淘宝
curl "http://localhost:8080/api/v1/products/search?keyword=香薰蜡烛&platform=淘宝"

# 仅搜索拼多多
curl "http://localhost:8080/api/v1/products/search?keyword=零食礼盒&platform=拼多多"
```

如果未配置某平台的密钥，后端日志会显示：`XX keys not configured, skipping XX search`——这意味着该平台被跳过，不属于报错。

---

## 项目结构

```
GiftGPT/
├── README.md
├── setup.bat                  # 一键启动
├── start-server.bat           # 启动后端
├── start-frontend.bat         # 启动前端
├── backend/                   # 后端
│   ├── pom.xml                # Maven 父 POM
│   ├── giftgpt-server/        # 启动入口
│   ├── giftgpt-common/        # 公共组件
│   ├── giftgpt-auth/          # 认证授权
│   ├── giftgpt-user/          # 用户与收礼人画像
│   ├── giftgpt-recommendation/ # AI 推荐引擎
│   ├── giftgpt-goods/         # 商品服务
│   ├── giftgpt-order/         # 订单与全链路
│   ├── giftgpt-content/       # 社区与日历
│   ├── giftgpt-enterprise/    # B端企业服务
│   └── giftgpt-ai/            # Python AI 服务
└── frontend/
    └── giftgpt-web/           # Next.js 前端
```

---

## API 概览

Base URL: `/api/v1`

| 模块 | 路径 | 说明 |
|------|------|------|
| 认证 | `POST /auth/register` `POST /auth/login` | 注册 / 登录 |
| 收礼人 | `GET/POST /recipients` `GET/PUT/DELETE /recipients/{id}` | 画像 CRUD |
| 推荐 | `POST /recommendations/search` `POST /recommendations/analyze` `POST /recommendations/ai-gifts` `POST /recommendations/match` `GET /recommendations/history` | 一次性推荐 / ①分析性格 / ②AI判断礼物 / ③平台搜索 / 历史 |
| 商品 | `GET /products/search` `GET /products/{id}` `GET /products/platforms/pinduoduo/authority` | 搜索 / 详情 / 拼多多授权状态 |
| 送礼 | `GET/POST /gifts` `POST /gifts/{id}/order` `GET /gifts/{id}/logistics` | 记录 / 下单 / 物流 |
| 贺卡 | `POST /greetings/generate` | AI 生成贺卡文案 |
| 社区 | `GET/POST /stories` `POST /stories/{id}/like` `POST /stories/{id}/unlike` `GET/POST /stories/{id}/replies` | 故事 / 点赞 / 取消点赞 / 评论 |
| 日历 | `GET/POST /calendar` | 日历提醒列表 / 创建 |
| 企业 | `POST /enterprise/register` `POST /enterprise/orders/batch` | 注册 / 批量下单 |

---

## 快速开始

### 环境要求

- JDK 11（运行）/ JDK 17（编译）
- Node.js 20+
- Maven 3.9+
- Python 3.11+ (可选)
- **Windows**: 需 JDK 11 运行（JDK 17 在部分 Windows 版本上有 WEPoll 兼容性 bug）

### 启动

```bash
# 自动构建并启动（首次运行推荐）
build-and-run.bat

# 或手动：
# 1. 启动后端
start-server.bat

# 2. 启动前端
start-frontend.bat
```

### 访问地址

| 服务 | 地址 |
|------|------|
| 前端 | http://localhost:3000 |
| API 文档 | http://localhost:8080/swagger-ui.html |
| H2 控制台 | http://localhost:8080/h2-console |
| Python AI (可选) | http://localhost:8000/docs |

> 数据库使用 H2 嵌入式数据库，数据存储在 `backend/giftgpt-server/data/`，无需额外安装。

---

## 数据库

### 核心表

| 表名 | 说明 |
|------|------|
| `user` | 用户表 |
| `recipient` + `recipient_tag` + `recipient_profile` | 收礼人画像 |
| `user_consume_profile` | 消费画像 |
| `gift_record` | 送礼记录 |
| `product` | 商品 |
| `order` | 订单 |
| `packaging` | 包装服务 |
| `greeting_card` | 电子贺卡 |
| `feedback` | 收礼人反馈 |
| `story` | 社区故事 |
| `calendar_event` | 日历提醒 |
| `data_authorization` | 数据授权记录 |
| `recommendation_history` | 推荐历史 |
| `enterprise` | 企业 |

---

## 开发路线图

- [x] Spring Boot 多模块项目骨架
- [x] 用户注册/登录 (JWT)
- [x] 收礼人画像 CRUD + 标签系统
- [x] AI 推荐引擎（Deepseek LLM 智能匹配礼物，带 fallback 降级）
- [x] 商品搜索与详情
- [x] 订单、贺卡、物流全链路
- [x] 社区故事与日历提醒
- [x] B端企业服务
- [x] 社区故事（发布、浏览、点赞/取消点赞、评论回复）
- [x] 日历提醒（添加事件、查看列表、历史记录）
- [x] 深色模式（右上角月亮/太阳图标切换）
- [x] 手机号唯一性校验（注册时提示"该手机号已被注册"）
- [x] 数据库持久化（H2 文件数据库 `data/giftgpt.mv.db`，重启数据不丢失）
- [x] 用户名持久化（JWT token 中携带昵称，页面刷新不丢失登录状态）
- [x] 跨域 CORS 修复（`CorsFilter` 在过滤器层处理预检，解决登录后接口 "Failed to fetch" / 误判未登录）
- [x] 帖子点赞单次限制（同账号对同一帖子只能点赞一次，再次点击取消）+ 评论回复修复（`story_reply` 表结构补齐 `update_time` 列）
- [x] 日历加载/创建修复（分页插件接入，事件正常展示）
- [x] 收礼人画像创建/列表修复（受 CORS 预检影响，现已恢复）
- [x] MyBatis-Plus 分页插件接入（`PaginationInnerInterceptor`，列表 `total`/分页正确）
- [x] 社区展示优化（联表返回发帖人昵称与"我是否已点赞"状态，时间显示为相对时间）
- [x] 界面美化（暖色背景 + 精炼玫瑰配色 + 柔和分层阴影 + 组件精修，降低模板感）
- [x] 第三方电商对接（京东联盟 / 淘宝联盟 / 多多进宝 API，AI 推荐匹配真实商品）
- [x] 拼多多授权流程修复（关键词搜索需 PID 站长授权；暴露 `GET /products/platforms/pinduoduo/authority` 授权链接，未授权时优雅跳过搜索而非报 50001；前端授权横幅）
- [x] 拼多多搜索修复（带 `custom_parameters` 即可搜索，无需站长授权；移除 `isAuthorized` 闸门，PID 持久化到 `data/pdd_pid.txt` 避免重启变更）
- [x] AI 推荐分步骤进度（①分析性格 → ②AI智能判断礼物 → ③各大平台搜索，每完成一步打钩；提示词优化；步骤③真实调用京东/淘宝/拼多多搜索匹配商品）
- [x] 退出登录自动跳转首页
- [ ] 京东 / 淘宝联盟审核通过后联调
- [ ] LLM 推理集成
- [ ] 知识图谱多跳推理

---

## 商业价值

| 收入来源 | 模式 |
|---------|------|
| CPS 佣金 | 按成交额抽佣 5%-15% |
| 包装增值 | 按单收费 ¥9.9-¥49.9 |
| B端SaaS | 按年/按人数订阅 |
| 广告推广 | 品牌新品首发推广 |
| 数据服务 | 脱敏消费趋势报告 |
