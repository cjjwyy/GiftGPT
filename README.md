...
- [x] AI 推荐分步骤进度（①分析性格 → ②AI智能判断礼物 → ③拼多多搜索，每完成一步打钩；提示词优化；步骤③真实调用拼多多搜索匹配商品）
- [x] 退出登录自动跳转首页
- [x] 精简为拼多多单一平台（移除京东/淘宝/得物服务与配置，`CommerceService` 仅调度 `PddService`）
- [x] 拼多多商品链接修复（优先 `goods_id` 拼 URL，兼容性优于 `goods_sign`）
- [x] 选品页移除平台选择器（仅展示拼多多商品，默认搜索"礼物"）
- [x] AI 推荐提示词优化（预算约束 60%-100%、推荐理由限 25 字、以"动词+称谓"开头、语言柔和温暖）
- [x] 推荐理由悬浮显示全文（`title` 属性 tooltip，`line-clamp-2` 截断）
- [x] 推荐历史记录压缩存储（GZIP + Base64，列表轻量返回，详情按需解压）
- [x] 推荐历史记录管理（编辑模式、全选、批量删除，用户仅可操作自己的记录）
- [x] 页面状态恢复（推荐页与选品页用 `sessionStorage` 保存最近一次结果，返回时恢复）
- [x] `recommendation_history` 表补齐 `update_time` 列（`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`）
- [x] 礼物包装定制（5 种礼盒 + 6 种个性化定制 + 4 种丝带绑法，AI 智能推荐，历史记录管理）
- [x] 包装方案保存修复（`packaging.order_id` 改为可空，支持独立包装方案不关联订单）
- [x] AI 包装推荐优化（仅根据商品信息推荐，不依赖收礼人画像；推荐后清空旧选择直接应用；不生成推荐理由）
- [x] 智能推荐礼物卡片按钮布局优化（包装与查看详情按钮纵向排列，避免文字换行）
- [ ] LLM 推理集成
- [ ] 知识图谱多跳推理
