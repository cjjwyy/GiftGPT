...
export default function RecipientsPage() {
  ...
  useEffect(() => {
    recipientApi.list()
      ...
      .catch((err: any) => { setLoading(false); toast.error(err.message || '加载失败，请先登录'); });
  }, []);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">收礼人画像</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">管理你的收礼人，让AI更精准推荐</p>
        </div>
      ...
      {recipients.length === 0 ? (
        <div className="card text-center py-20">
          ...
          <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400 mb-2">还没有收礼人画像</h3>
          <p className="text-gray-400 dark:text-gray-500 mb-4">添加第一位收礼人，开始AI选礼之旅</p>
            ...
            <div key={r.id} className="card hover:shadow-md transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <Link href={`/recipients/${r.id}`} className="p-1.5 hover:bg-gray-100 rounded-lg"><Pencil className="w-4 h-4 text-gray-400 dark:text-gray-500" /></Link>
                  <button onClick={() => handleDelete(r.id)} className="p-1.5 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4 text-gray-400 dark:text-gray-500" /></button>
                </div>
              </div>
              <h3 className="font-semibold text-gray-800 dark:text-gray-100">{r.name}</h3>
              {r.relation && <p className="text-sm text-gray-500 dark:text-gray-400">{r.relation}</p>}
